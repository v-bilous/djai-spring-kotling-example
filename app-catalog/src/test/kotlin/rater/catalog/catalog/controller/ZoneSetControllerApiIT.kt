...
class ZoneSetControllerApiIT : AbstractIntegrationTest<CatalogZoneSet>() {

	private val url = "/zoneset"

	@Autowired
	lateinit var repository: ZoneSetControllerRepository

	@Test
	fun `deleteAllUsingDELETE31 api`() {
	...
	@Test
	fun `getByIdUsingGET39 with required fields`() {
		val res = createWithRequiredFields()
		val savedRes = repository.save(res)
	...
	@Test
	fun `getByIdUsingGET39 with all fields`() {
	...
	@Test
	fun `saveAllUsingPOST34 with required fields`() {
		val res = createWithRequiredFields()
	...
	@Test
	fun `saveAllUsingPOST34 with all fields`() {
		val res = createWithAllFields()
		val result = super.create(url, res)
		val savedRes = repository.getById(findIdentityId(result))
		resourceAsserts(savedRes, result)
	...
	@Test
	fun `saveUsingPOST33 with required fields`() {
		val res = createWithRequiredFields()
		val result = super.create(url, res)
		val savedRes = repository.getById(findIdentityId(result))
		resourceAsserts(savedRes, result)
	...
	@Test
	fun `saveUsingPOST33 with all fields`() {
		val res = createWithAllFields()
		val result = super.create(url, res)
		val savedRes = repository.getById(findIdentityId(result))
		resourceAsserts(savedRes, result)
	...
	@Test
	fun `updateUsingPUT32 with required fields`() {
		val res = createWithRequiredFields()
		val savedRes = repository.save(res)
	...
	@Test
	fun `updateUsingPUT32 with all fields`() {
		val res = createWithAllFields()
		val savedRes = repository.save(res)
	...
	private fun resourceAsserts(savedResource: CatalogZoneSet, result: MvcResult, index: Int = -1) {
		...
		assertListsEquals(savedResource.zone, getValue(result, "$prefix.zone"))
	}

	private fun createWithRequiredFields(): CatalogZoneSet {
	return CatalogZoneSet(
				createdDate = java.sql.Timestamp(System.currentTimeMillis()),
				description = null,
				eligibilitySet = setOf("aaaaaaaa-bbbb-cccc-dddd-eeeeeeffffff".toUUID()),
				isDeleted = false,
				name = null,
				updatedDate = null,
				zone = null
	...
	private fun createWithAllFields(): CatalogZoneSet {
		return CatalogZoneSet(
				createdDate = java.sql.Timestamp(System.currentTimeMillis()),
