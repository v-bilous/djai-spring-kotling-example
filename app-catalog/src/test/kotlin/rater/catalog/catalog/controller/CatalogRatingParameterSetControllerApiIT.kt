...
class CatalogRatingParameterSetControllerApiIT : AbstractIntegrationTest<CatalogRatingParameterSet>() {

	private val url = "/rating-parameter-sets"

	@Autowired
	lateinit var repository: CatalogRatingParameterSetControllerRepository

	@Test
	fun `deleteAllUsingDELETE20 api`() {
	...
	@Test
	fun `getByIdUsingGET25 with required fields`() {
		val res = createWithRequiredFields()
		val savedRes = repository.save(res)
	...
	@Test
	fun `getByIdUsingGET25 with all fields`() {
	...
	@Test
	fun `saveAllUsingPOST20 with required fields`() {
		val res = createWithRequiredFields()
	...
	@Test
	fun `saveAllUsingPOST20 with all fields`() {
		val res = createWithAllFields()
		val result = super.create(url, res)
		val savedRes = repository.getById(findIdentityId(result))
		resourceAsserts(savedRes, result)
	...
	@Test
	fun `saveUsingPOST21 with required fields`() {
		val res = createWithRequiredFields()
		val result = super.create(url, res)
		val savedRes = repository.getById(findIdentityId(result))
		resourceAsserts(savedRes, result)
	...
	@Test
	fun `saveUsingPOST21 with all fields`() {
		val res = createWithAllFields()
		val result = super.create(url, res)
		val savedRes = repository.getById(findIdentityId(result))
		resourceAsserts(savedRes, result)
	...
	@Test
	fun `updateUsingPUT20 with required fields`() {
		val res = createWithRequiredFields()
		val savedRes = repository.save(res)
	...
	@Test
	fun `updateUsingPUT20 with all fields`() {
		val res = createWithAllFields()
		val savedRes = repository.save(res)
	...
	private fun resourceAsserts(savedResource: CatalogRatingParameterSet, result: MvcResult, index: Int = -1) {
		...
		assertListsEquals(savedResource.ratingParameterIds, getValue(result, "$prefix.ratingParameterIds"))
	...
	private fun createWithRequiredFields(): CatalogRatingParameterSet {
	return CatalogRatingParameterSet(
				createdDate = java.sql.Timestamp(System.currentTimeMillis()),
				description = null,
				isDeleted = false,
				name = "test string value",
				ratingParameterIds = listOf("aaaaaaaa-bbbb-cccc-dddd-eeeeeeffffff".toUUID()),
				realm = "test string value",
				updatedDate = null
		)
	}

	private fun createWithAllFields(): CatalogRatingParameterSet {
		return CatalogRatingParameterSet(
				createdDate = java.sql.Timestamp(System.currentTimeMillis()),
