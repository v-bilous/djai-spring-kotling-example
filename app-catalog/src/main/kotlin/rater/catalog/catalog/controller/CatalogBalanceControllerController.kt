package rater.catalog.catalog.controller

import rater.catalog.catalog.domain.CatalogBalance
import java.util.UUID
import rater.catalog.catalog.service.CatalogBalanceControllerService
import rater.catalog.catalog.controller.api.CatalogBalanceControllerApi
import rater.catalog.controller.AbstractController
import org.springframework.data.domain.Page
import org.springframework.data.domain.Pageable
import org.springframework.data.domain.Sort
import org.springframework.data.web.PageableDefault
import org.springframework.http.ResponseEntity
import org.springframework.web.bind.annotation.*

@javax.annotation.Generated(value = ["org.openapitools.codegen.CodeCodegen"])

@RestController
class CatalogBalanceControllerController(service: CatalogBalanceControllerService)
    : AbstractController<CatalogBalance, CatalogBalanceControllerService>(service), CatalogBalanceControllerApi {


    override fun deleteAllUsingDELETE1(
			@RequestParam(value = "ids", required = true) ids: List<UUID>): ResponseEntity<List<CatalogBalance>> {
        return ResponseEntity.ok(super.deleteAll(ids))
    }

    override fun deleteUsingDELETE1(
			@PathVariable("id") id: UUID): ResponseEntity<CatalogBalance> {
        return super.delete(id)
    }

    override fun getAllUsingGET2(
			@RequestParam(value = "offset", required = false) offset: Long?,
			@RequestParam(value = "pageNumber", required = false) pageNumber: Int?,
			@RequestParam(value = "pageSize", required = false) pageSize: Int?,
			@RequestParam(value = "paged", required = false) paged: Boolean?,
			@RequestParam(value = "search", required = false) search: String?,
			@RequestParam(value = "sort.sorted", required = false) sortSorted: Boolean?,
			@RequestParam(value = "sort.unsorted", required = false) sortUnsorted: Boolean?,
			@RequestParam(value = "unpaged", required = false) unpaged: Boolean?,
			@PageableDefault(value=0, size = 50, sort=["id"], direction = Sort.Direction.ASC) page: Pageable): ResponseEntity<Page<CatalogBalance>> {
        return getAll(search, page)
    }

    override fun getByIdUsingGET3(
			@PathVariable("id") id: UUID): ResponseEntity<CatalogBalance> {
        return super.getById(id)
    }

    override fun saveAllUsingPOST1(
			@RequestBody catalogBalance: List<CatalogBalance>): ResponseEntity<List<CatalogBalance>> {
        return ResponseEntity.ok(super.saveAll(catalogBalance))
    }

    override fun saveUsingPOST1(
			@RequestBody catalogBalance: CatalogBalance): ResponseEntity<CatalogBalance> {
        return super.create(catalogBalance)
    }

    override fun updateUsingPUT1(
			@PathVariable("id") id: UUID,
			@RequestBody catalogBalance: CatalogBalance): ResponseEntity<CatalogBalance> {
        return super.update(id, catalogBalance)
    }

}
